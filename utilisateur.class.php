 ...
 * de passe sont hachés avec `md5()` avant d'être enregistrés en base.
 */
class Utilisateur
{
     ...
     * Le mot de passe est haché avant d'être inséré en base.
     *
    ...
    public function inscription(): void
    {
        ...
        // Hachage du mot de passe avant l'insertion en base
        $passwordHache = hash('md5', $this->password);
     ...
     * et compare le mot de passe haché (à l'aide de `md5`) fourni à celui enregistré en base.
     *
    ...
    public function authentification(): bool
    {
        ...
        /* Récupération des informations de l'utilisateur en base de données
           ...
           $donneesUtilisateurEnBase['password'] : le mot de passe haché et stocké en base pour l'utilisateur */
        $donneesUtilisateurEnBase = $requete->fetch(PDO::FETCH_ASSOC);

        // Vérifie si l'utilisateur en base existe
        if ($donneesUtilisateurEnBase)
        {
            // Hachage du mot de passe saisi pour la comparaison
            $passwordHache = hash('md5', $this->getPassword());
